SUBROUTINE  meshtools
  ...
  END Do
  !!---------------------------------------------------
  !! renumerotation des sommets interieurs de 1 � NsInt
  !! --------------------------------------------------
  ...
  !!=================================================================================
  !! A ce stade nous avons renumerote les inconnues � l'interieur 
  ...
  ! -------------------------------------------------------------------------------------
  ! Pour chaque segment iseg, NsigK(Nseg) designe le normal � l'interface primale sortant
  !  de K alors que NsigeKe(Nseg) designe le normal � l'interface duale sortant de Ke
  ! -------------------------------------------------------------------------------------
  ALLOCATE(NsigK(2,Nseg),NsigeKe(2,Nseg))
  !
  DO iseg = 1, Nseg
     is = NuSeg(1,iseg); js = NuSeg(2,iseg)  !! is,js num�ro globale
     !! 1,2 num�ro locale
     iK = NumTVoisSeg(1,iseg); jL = NumTVoisSeg(2,iseg) !! ik,jL num�ro globale
     !! 1,2 num�ro locale
     !!
     !
     IF (NTypSeg(iseg) == 0)  THEN
        ...
        ! ici on effectue le prouduit scalaire pour v�rifier la bonne
     ...
     ELSE
        ...
        !-------------------------------------------------------
        NsigeKe(1,iseg) = -(y(4)-y(3)) ; NsigeKe(2,iseg) = x(4)-x(3)
        ! ici on effectue le prouduit scalaire pour v�rifier
